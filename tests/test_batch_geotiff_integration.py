...
class TestMultispectralGeoTiffPair:
    ...
    def test_sentinel2_band_descriptions_determine_rgb_band_order(
            self,
            pair_dirs: Path,
        ) -> None:
            """Sentinel-2 band descriptions determine automatic RGB selection."""
            before = np.zeros(
                (4, HEIGHT, WIDTH),
                dtype=np.uint8,
            )
            ...
            # Actual raster order: B08, B04, B03, B02.
            before[0] = 200
            before[1] = LAND_RGB[0]
            before[2] = LAND_RGB[1]
            before[3] = LAND_RGB[2]
            ...
            for path, data in (
                    (
                            pair_dirs / "before" / "sentinel2.tif",
                            before,
                    ),
                    (
                            pair_dirs / "after" / "sentinel2.tif",
                            after,
                    ),
            ):
                with rasterio.open(
